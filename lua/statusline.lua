...
local modules = statusline_require.lazy_require {
  ...
  loader = 'statusline.utils.loader',
  utils_section = 'statusline.utils.section',
  utils = 'statusline.utils.utils',
  utils_notices = 'statusline.utils.notices',
  config_module = 'statusline.config',
  nvim_opts = 'statusline.utils.nvim_opts',
}
...
-- The events on which statusline redraws itself
...
local function apply_transitional_separators(status, is_focused)
  ...
  while str_checked ~= nil do
    ...
    elseif next_char == '=' and last_hl and (last_hl:find('^statusline_a') or last_hl:find('^statusline_b')) then
      -- TODO: Fix this properly
      -- This check for statusline_a and statusline_b is dumb. It doesn't guarantee
...
local statusline = modules.utils.retry_call_wrap(function(sections, is_focused, is_winbar)
  ...
  for _, section_name in ipairs(section_sequence) do
    if sections['statusline_' .. section_name] then
      -- insert highlight+components of this section to status_builder
      local section_data =
        modules.utils_section.draw_section(sections['statusline_' .. section_name], section_name, is_focused)
...
Please report the issue at https://github.com/nvim-jo/statusline.nvim/issues .
...
--- Also sets up auto command to reload statusline on ColorScheme or background changes.
local function setup_theme()
  ...
  vim.cmd([[autocmd statusline ColorScheme * lua require'statusline'.setup()
    autocmd statusline OptionSet background lua require'statusline'.setup()]])
end

---@alias StatusDispatchSecs
---| 'sections'
---| 'winbar'
--- generates statusline.statusline & statusline.winbar function
...
---@alias StatuslineRefreshOptsKind
---| 'all'
---| 'tabpage'
---| 'window'
---@alias StatuslineRefreshOptsPlace
---| 'statusline'
---| 'tabline'
---| 'winbar'
---@class StatuslineRefreshOpts
---@field scope StatuslineRefreshOptsKind
---@field place StatuslineRefreshOptsPlace[]
---@field trigger 'autocmd'|'autocmd_redired'|'timer'|'unknown'
--- Refresh contents of statusline
---@param opts StatuslineRefreshOpts
...
--- Sets &tabline option to statusline
...
local function set_tabline(hide)
  ...
  vim.cmd([[augroup statusline_tal_refresh | exe "autocmd!" | augroup END]])
  if not hide and next(config.tabline) ~= nil then
    vim.loop.timer_start(
      timers.tal_timer,
      0,
      config.options.refresh.tabline,
      modules.utils.timer_call(timers.tal_timer, 'statusline_tal_refresh', function()
        refresh { kind = 'tabpage', place = { 'tabline' }, trigger = 'timer' }
      end, 3, 'statusline: Failed to refresh tabline')
    )
    modules.utils.define_autocmd(
      default_refresh_events,
      '*',
      "call v:lua.require'statusline'.refresh({'kind': 'tabpage', 'place': ['tabline'], 'trigger': 'autocmd'})",
      'statusline_tal_refresh'
    )
...
--- Sets &statusline option to statusline
--- adds auto command to redraw statusline on VimResized event
...
local function set_statusline(hide)
  ...
  vim.cmd([[augroup statusline_stl_refresh | exe "autocmd!" | augroup END]])
  if not hide and (next(config.sections) ~= nil or next(config.inactive_sections) ~= nil) then
    ...
    if config.options.globalstatus then
      ...
      vim.loop.timer_start(
        ...
        modules.utils.timer_call(timers.stl_timer, 'statusline_stl_refresh', function()
          refresh { kind = 'window', place = { 'statusline' }, trigger = 'timer' }
        end, 3, 'statusline: Failed to refresh statusline')
      )
      modules.utils.define_autocmd(
        default_refresh_events,
        '*',
        "call v:lua.require'statusline'.refresh({'kind': 'window', 'place': ['statusline'], 'trigger': 'autocmd'})",
        'statusline_stl_refresh'
      )
    else
      ...
      modules.utils.define_autocmd(
        default_refresh_events,
        '*',
        "call v:lua.require'statusline'.refresh({'kind': 'tabpage', 'place': ['statusline'], 'trigger': 'autocmd'})",
        'statusline_stl_refresh'
      )
    end
...
--- Sets &winbar option to statusline
...
local function set_winbar(hide)
  ...
  vim.cmd([[augroup statusline_wb_refresh | exe "autocmd!" | augroup END]])
  if not hide and (next(config.winbar) ~= nil or next(config.inactive_winbar) ~= nil) then
    vim.loop.timer_start(
      timers.wb_timer,
      0,
      config.options.refresh.winbar,
      modules.utils.timer_call(timers.wb_timer, 'statusline_wb_refresh', function()
        refresh { kind = 'tabpage', place = { 'winbar' }, trigger = 'timer' }
      end, 3, 'statusline: Failed to refresh winbar')
    )
    modules.utils.define_autocmd(
      default_refresh_events,
      '*',
      "call v:lua.require'statusline'.refresh({'kind': 'tabpage', 'place': ['winbar'], 'trigger': 'autocmd'})",
      'statusline_wb_refresh'
    )
...
---@alias StatuslineHideOptsPlace
---| 'statusline'
---| 'tabline'
---| 'winbar'
---@class StatuslineHideOpts
---@field place StatuslineHideOptsPlace[]
---@field unhide boolean
---@param opts StatuslineHideOpts
...
Statusline supports neovim 0.7 and up. It seems you're using a older version.
Please update to newer version. Or if you have atleast neovim 0.5 you
can use older compatible versions of statusline using compat tags like
...
--- statusline config is done in several setup calls as chunks. This way
...
local function setup(user_config)
  if package.loaded['statusline.utils.notices'] then
  ...
  if verify_nvim_version() then
    ...
    vim.cmd([[augroup statusline | exe "autocmd!" | augroup END]])
    setup_theme()
