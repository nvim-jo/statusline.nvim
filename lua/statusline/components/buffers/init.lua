...
local require = require('statusline_require').require
local Buffer = require('statusline.components.buffers.buffer')
local M = require('statusline.component'):extend()
local highlight = require('statusline.highlight')
...
local function get_hl(section, is_active)
  ...
  local section_redirects = {
    statusline_x = 'statusline_c',
    statusline_y = 'statusline_b',
    statusline_z = 'statusline_a',
  }
...
function M:init(options)
  ...
  local default_active = options.use_mode_colors
      and function()
        return get_hl('statusline_' .. options.self.section, true)
      end
    or get_hl('statusline_' .. options.self.section, true)
  default_options.buffers_color = {
    active = default_active,
    inactive = get_hl('statusline_' .. options.self.section, false),
  }
...
vim.cmd([[
  function! StatuslineSwitchBuffer(bufnr, mouseclicks, mousebutton, modifiers)
    execute ":buffer " . a:bufnr
  endfunction

  command! -nargs=1 -bang StatuslineBuffersJump call v:lua.require'statusline.components.buffers'.buffer_jump(<f-args>, "<bang>")
]])
