...
local modules = statusline_require.lazy_require {
  default_config = 'statusline.components.diagnostics.config',
  sources = 'statusline.components.diagnostics.sources',
...
function M:init(options)
  ...
  if #self.options.sources < 1 then
    modules.utils_notices.add_notice(
      '### diagnostics.sources\n\nno sources for diagnostics configured.\nPlease specify which diagnostics source you want statusline to use with `sources` option.\n'
    )
  end
end
