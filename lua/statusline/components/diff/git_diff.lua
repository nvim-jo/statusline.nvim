...
function M.init(opts)
  if type(opts.source) == 'function' then
    ...
    modules.utils.define_autocmd('BufEnter', "lua require'statusline.components.diff.git_diff'.update_diff_args()")
    modules.utils.define_autocmd('BufWritePost', "lua require'statusline.components.diff.git_diff'.update_git_diff()")
    M.update_diff_args()
  end
end
...
function M.get_sign_count(bufnr)
  ...
  elseif vim.g.actual_curbuf ~= nil and active_bufnr ~= vim.g.actual_curbuf then
    -- Workaround for https://github.com/nvim-statusline/statusline.nvim/issues/286
