...
local modules = statusline_require.lazy_require {
  ...
  color_utils = 'statusline.utils.color_utils',
}
...
-- table to store the highlight names created by statusline
...
function M.get_mode_suffix()
  local mode = require('statusline.utils.mode').get_mode()
...
function M.get_statusline_hl(name)
...
---@param section string the statusline section component is in.
...
local function get_default_component_color(hl_name, mode, section, color, options)
  ...
  if not ret.fg or not ret.bg then
    apply_default(default_theme_color, string.format('statusline_%s_%s', section, mode))
  end
...
function M.create_component_highlight_group(color, highlight_tag, options, apply_no_default)
  ...
  local tag_id = 0
  while
    M.highlight_exists(table.concat({ 'statusline', section, highlight_tag }, '_'))
    or (section and M.highlight_exists(table.concat({ 'statusline', section, highlight_tag, 'normal' }, '_')))
  do
  ...
  if type(color) == 'string' then
    local highlight_group_name = table.concat({ 'statusline', section, highlight_tag }, '_')
  ...
  for _, mode in ipairs(modes) do
    local hl_name = table.concat({ 'statusline', section, highlight_tag, mode }, '_')
    local cl = color
  ...
  return {
    name = table.concat({ 'statusline', section, highlight_tag }, '_'),
...
function M.get_transitional_highlights(left_hl, right_hl)
  ...
  local highlight_name = table.concat({ 'statusline_transitional', left_hl, 'to', right_hl }, '_')
