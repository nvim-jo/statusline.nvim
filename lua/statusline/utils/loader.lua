...
local require = statusline_require.require
local modules = statusline_require.lazy_require {
  utils = 'statusline.utils.utils',
  notice = 'statusline.utils.notices',
  fn_store = 'statusline.utils.fn_store',
}
local is_valid_filename = statusline_require.is_valid_filename
local sep = statusline_require.sep
...
local component_types = {
  ...
  lua_fun = function(component)
    return require('statusline.components.special.function_component')(component)
  end,
  --- loads lua modules as components (ones in /lua/statusline/components/)
  mod = function(component)
    local ok, loaded_component = pcall(require, 'statusline.components.' .. component[1])
    if ok then
      ...
      elseif type(loaded_component) == 'function' then
        ...
        loaded_component = require('statusline.components.special.function_component')(component)
      end
  ...
  var = function(component)
    return require('statusline.components.special.vim_var_component')(component)
  end,
  --- loads vim functions and lua expressions as components
  ['_'] = function(component)
    return require('statusline.components.special.eval_func_component')(component)
  end,
}
  ...
  for section_name, section in pairs(sections) do
    for index, component in pairs(section) do
      ...
      if is_valid_component_type(index, component) then
        component.self = {}
        component.self.section = section_name:match('statusline_(.*)')
        -- apply default args
...
local function load_extensions(config)
  ...
  for _, extension in pairs(config.extensions) do
    if type(extension) == 'string' then
      local ok
      ok, extension = pcall(require, 'statusline.extensions.' .. extension)
      if not ok then
        modules.notice.add_notice(string.format(
          [[
### Extensions
...
local function load_all(config)
  require('statusline.component')._reset_components()
  modules.fn_store.clear_fns()
  require('statusline.utils.nvim_opts').reset_cache()
...
local function load_theme(theme_name)
  ...
  local retval
  local path = table.concat { 'lua/statusline/themes/', theme_name, '.lua' }
  local files = vim.api.nvim_get_runtime_file(path, true)
  if #files <= 0 then
    path = table.concat { 'lua/statusline/themes/', theme_name, '/init.lua' }
  ...
  elseif n_files == 1 then
    ...
    -- More then 1 found . Use the first one that isn't in statuslines repo
    local statusline_repo_pattern = table.concat({ 'statusline.nvim', 'lua', 'statusline' }, sep)
    local file_found = false
    for _, file in ipairs(files) do
      if not file:find(statusline_repo_pattern) then
        retval = dofile(file)
        file_found = true
        break
      end
    ...
    if not file_found then
      ...
      -- appear to be in statuslines repo . Just run the first one
      retval = dofile(files[1])
    end
  end
  return retval
