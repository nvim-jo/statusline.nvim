...
function M.notice_message_startup()
  ...
  vim.defer_fn(function()
    ...
    if #notices > 0 or #persistent_notices > 0 then
      vim.cmd('command! -nargs=0 StatuslineNotices lua require"statusline.utils.notices".show_notices()')
      vim.notify(
        'statusline: There are some issues with your config. Run :StatuslineNotices for details',
        vim.log.levels.WARN,
        {}
      )
    end
...
function M.show_notices()
  ...
  local ok, _ = pcall(vim.api.nvim_buf_set_name, 0, 'Statusline Notices')
  if not ok then
    vim.notify('Statusline Notices is already open in another buffer', vim.log.levels.ERROR, {})
    vim.cmd('normal q')
    return
  end
