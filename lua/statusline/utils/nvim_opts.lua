...
--   }
-- }
---@class StatuslineNvimOptCacheOptStore
---@field prev any
---@field set any
---@alias StatuslineNvimOptCacheOpt table<string, StatuslineNvimOptCacheOptStore>
---@class StatuslineNvimOptCache
---@field global StatuslineNvimOptCacheOptStore[]
---@field buffer table<number, StatuslineNvimOptCacheOptStore[]>
---@field window table<number, StatuslineNvimOptCacheOptStore[]>
---@type StatuslineNvimOptCache
...
local function set_opt(name, val, getter_fn, setter_fn, cache_tbl)
  ...
  if cache_tbl[name].set ~= cur then
    if type(cur) ~= 'string' or not cur:find('statusline') then
...
function M.restore(name, opts)
  if opts == nil or opts.global then
    if options.global[name] ~= nil and options.global[name].prev ~= nil then
      ...
      if type(restore_to) == 'string' and restore_to:find('statusline') then
        restore_to = ''
      end
