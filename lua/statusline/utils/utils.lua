...
function M.extract_highlight_colors(color_group, scope)
  local color = require('statusline.highlight').get_statusline_hl(color_group)
  if not color then
...
local function autocmd_is_defined(event, pattern, command_str)
  return vim.api.nvim_exec(string.format('au statusline %s %s', event, pattern), true):find(command_str) ~= nil
end
...
---@param group   string group name defaults to statusline
function M.define_autocmd(event, pattern, cmd, group)
  ...
  if not autocmd_is_defined(event, pattern, cmd) then
    vim.cmd(string.format('autocmd %s %s %s %s', group or 'statusline', event, pattern, cmd))
  end
end
...
--- Check if comp is a statusline component
--- @param comp any
--- @return boolean
function M.is_component(comp)
  ...
  return mt and mt.__is_statusline_component == true
end
